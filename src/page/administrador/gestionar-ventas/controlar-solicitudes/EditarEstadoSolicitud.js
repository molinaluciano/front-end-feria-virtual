...
import { getStatusRequest } from '../../../../service/status_and_types/status_and_types';
...
function EditarEstadoSolicitud() {
    ...
    const requiredFields = ['idEstadoSolicitud'];
    ...
    const handleSave = async () => {
        if (requiredFields.some((field) => formData[field] === '')) {
            ...
            console.log(
                '🚀 ~ file: EditarEstadoSolicitud.js ~ line 31 ~ handleSave ~ formData',
                formData
            );
            const resultUpdated = await changeRequestStatus(
                formData.idSolicitud,
                formData.idEstadoSolicitud
            );
    ...
    const displayStatusRequest = statusRequest.map((status) => {
        return (
            <option value={status.idEstadoSolicitud}>
                {status.descripcion}
            </option>
        );
    });
...
export default EditarEstadoSolicitud;
