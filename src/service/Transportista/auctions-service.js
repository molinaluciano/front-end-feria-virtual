...
export const getAuctionsByCarrier = async (carrierId) => {
    ...
    const filterAuctionByTruck = (truck) => {
        const filteredAuctionByTruck = getAuctions.filter(
            (auction) => auction.camionSeleccionado == truck
        );
...
export const getAuctionDetailById = async (auctionId) => {
    ...
    const selectedProducerId = await getRequestByIdResponse[0]
        .productorSeleccionado;
    const producerInfo = getProducers.filter(
        (x) => x.idUsuario == selectedProducerId
    );
